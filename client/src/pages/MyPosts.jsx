...
export default function MyPosts() {
  ...
  const fetchPosts = async () => {
    const res = await axios.get('https://monochrome-w1vb.onrender.com/api/posts/mine', { headers: { Authorization: `Bearer ${token}` } });
    setPosts(res.data);
  };
  ...
  const handleDelete = async (id) => {
    ...
    await axios.delete(`https://monochrome-w1vb.onrender.com/api/posts/${id}`, { headers: { Authorization: `Bearer ${token}` } });
    fetchPosts();
  };
